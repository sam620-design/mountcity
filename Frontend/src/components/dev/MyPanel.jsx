        ...
        {tab === 7 && (() => {
          ...
          customers.forEach(c => {
            ...
            sortedPmts.forEach(p => {
              ...
              logs.push({ type: 'PAYIN', date: new Date(p.date), customer: cName, advisor: adv, amount: pAmt, note: p.note || '', detail: `Payment — ${c.projectName} Plot ${c.plotNumber}` });
            });
            (c.advisorPayouts || []).forEach(p => {
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filteredLogs.map((l, li) => (
                      <tr key={li} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className={`px-5 py-3 text-right font-black ${l.type === 'PAYOUT' ? 'text-red-600' : (l.type === 'PAYIN' || l.type === 'BOOKING' ? 'text-emerald-600' : 'text-slate-700')}`}>{l.amount > 0 ? inr(l.amount) : '—'}</td>
